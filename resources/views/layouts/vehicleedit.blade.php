    ...
    <div class="section__content section__content--p30">
        <div class="container-fluid m-auto"> 
            ...
            <div class="col-lg-7 m-auto">
                @foreach ($vehicles as $items)
                
                <form action="/vehicle/{{$items->id}}" method="POST">
                   @csrf
                    @method('PATCH')
                    <div class="card">
                        <div class="card-body card-block">
                            <div class="form-group">
                                ...
                                <input type="text" name="license_plate" id="license_plate" placeholder="Enter License Plate" class="form-control" required value="{{$items->license_plate}}">
                            </div>
                            
                            <div class="form-group">
                                <label for="member_id" class=" form-control-label">Owner</label>
                                <select name="member_id" id="select"  class="form-control" >
                                    <option value="{{$items->member_id}}">{{$items->name}}</option>
                            ...
                            <div class="row form-group">
                                <div class="col-8">
                                    <div class="form-group">
                                        ...
                                        <textarea name="notes" id="notes" class="form-control" rows="5">{{$items->notes}}</textarea>
                                    </div>
                                </div>
                        </div>
                        @endforeach
