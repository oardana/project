...
<div class="p-4 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-4 bg-gray-200 h-[100vh]">
    <h1 class="text-center text-3xl mb-10"> Parking Data</h1>
    ...
    @if($parkingdata->isEmpty())
     <h1 class="text-center text-3xl text-red-500 py-60">Data Not found.......</h1>
    @else
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-900 uppercase dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Type Member</th>
                    <th scope="col" class="px-6 py-3">Type Vehicle</th>
                    <th scope="col" class="px-6 py-3">Value</th>
                    <th scope="col" class="px-6 py-3">Date In</th>
                    <th scope="col" class="px-6 py-3">Date Out</th>
            ...
            <tbody>
                @foreach($parkingdata as $item)
                        <tr class="bg-white dark:bg-gray-800">
                            ...
                            <td class="px-6 py-4">{{$item->vehicle->member->name}}</td>
                            <td class="px-6 py-4">{{$item->hourlyrate->membership->name_member}}</td>
                            <td class="px-6 py-4">{{$item->hourlyrate->vehicletype->name_type}}</td>
                            <td class="px-6 py-4">{{$item->hourlyrate->value}}</td>
                            <td class="px-6 py-4">{{$item->date_in}}</td>
                            <td class="px-6 py-4">{{$item->date_out}}</td>
                            <td class="px-6 py-4">{{$item->amount_to_pay}}</td>
                            ...
                            <td class="px-6 py-4">
                                <div> 
                                    <a href="#" class="pl-3 font-medium text-red-600 dark:text-red-500 hover:underline" data-toggle="modal" data-target="#confirmDeleteModal{{$item->id}}">remove</a>
                                               ...
                                               <!-- Modal -->
                            <div class="fixed z-50 inset-0 overflow-y-auto" id="confirmDeleteModal{{$item->id}}" style="display: none;">
    ...
    @endif()
    {{$parkingdata->links('vendor.pagination.tailwind')}} 
</div>
@endsection